...
class Directory extends FileSystemNode {
    ...
    FileSystemNode getChild(String name) {
        ...
        if (node == null) {
            throw new RuntimeException("Path not found: " + name);
        }
        return node;
    }
}
...
class FileSystem {
    ...
    public void mkdir(String path) {
        ...
        if (idx < 0) throw new RuntimeException("Invalid path");
...
class Shell {
    public static void main(String[] args) {
        ...
        fs.mkdir("/a/b/c");
        fs.createFile("/a/hello.txt", "hello world");

        fs.list("/a");                 // hello.txt
        fs.readFile("/a/hello.txt");   // hello world
        fs.removeFile("/a/hello.txt");
        fs.list("/a");                 // empty
    }
}
