package medium.loggersystem.multilogger;
...
class FileAppender implements LogAppender{
    ...
    @Override
    public void append(String message){
        System.out.println("Appending in file "+ message );
    }
}
...
class CloudAppender implements LogAppender{
    @Override
    public void append(String message){
        System.out.println("Appendin the cloud" + message);
    }
}
...
class Logger {
    ...
    Logger() {
        ...
        LogAppender fileAppender = new FileAppender();
        LogAppender cloudAppender = new CloudAppender();

        // 3 handler
        LogHandler debugHandler = new DebugHandler(consoleAppender);
        LogHandler infoHandler = new InfoHandler(consoleAppender);
        LogHandler erroFileHandler = new ErrorHandler(fileAppender);
        LogHandler errorHandler = new ErrorHandler(consoleAppender);

        debugHandler.setNext(infoHandler).setNext(errorHandler).setNext(erroFileHandler);
